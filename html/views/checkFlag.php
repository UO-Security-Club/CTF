...
if (isset($_POST["flag"]) && isset($_POST["chal_flag"])) {

        $POSTflag = htmlspecialchars($_POST["flag"]);
        $Chal = htmlspecialchars($_POST["chal_flag"]);
        check_flag($POSTflag, $Chal, $usr);
}
...
function check_flag($sPOSTflag, $sChal, $usr){
	$server = "localhost";
	$username = "root";
	...
	if ($result->num_rows > 0) {
		...
		if ($isFlag == 1 && loginStatus() === 2) {
			echo "<br>Congrats! You got the flag!";
		}
		elseif ($isFlag == 1) {
